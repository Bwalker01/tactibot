...
export class DatabaseController {
    ...
    public async queryAll<T extends QueryResultRow>(queryString: string, params?: any[]): Promise<{ rows: T[] }> {
        const result = await this.pool.query<T>(queryString, params);
        return { rows: result.rows };
    }

    public async queryOne<T extends QueryResultRow>(queryString: string, params?: any[]): Promise<T | null> {
        const result = await this.pool.query<T>(queryString, params);
